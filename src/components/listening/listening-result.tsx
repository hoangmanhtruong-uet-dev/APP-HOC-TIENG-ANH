...
import type { getListeningPracticeResult } from "@/server/listening/content";

type ResultData = NonNullable<
  Awaited<ReturnType<typeof getListeningPracticeResult>>
>;

export function ListeningResult({ data }: { data: ResultData }) {
    ...
    <div className="mx-auto min-h-[100dvh] max-w-3xl space-y-8 bg-[#fbf9ff] px-4 py-6 pb-24 sm:px-6 lg:min-h-0 lg:rounded-3xl lg:border lg:border-[#e4deef] lg:p-8">
      <header className="rounded-2xl border border-[#e2dced] bg-white p-5">
        ...
        <p className="mt-2 text-sm text-[var(--muted-foreground)]">
          Điểm, correctness, thời gian nộp và trạng thái trễ đều do PostgreSQL
          xác định.
        </p>
      </header>

      <section aria-labelledby="listening-review-title">
        <h2 id="listening-review-title" className="text-2xl font-bold">
          Review từng câu
        </h2>
      ...
      {result.reviewAllowed ? (
        <section
          aria-labelledby="listening-transcript-title"
          className="rounded-xl border border-[var(--border-strong)] bg-[var(--surface)] p-5 sm:p-6"
        >
          <h2 id="listening-transcript-title" className="text-2xl font-bold">
            Transcript
          </h2>
          <p className="mt-2 text-sm text-[var(--muted-foreground)]">
            Transcript chỉ được PostgreSQL trả về sau submit.
          </p>
          <div className="mt-5 leading-7 break-words">
            <LessonMarkdown>{result.transcriptMarkdown}</LessonMarkdown>
          </div>
        </section>
      ) : null}
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <Button asChild>
          <Link href={`/practice/listening/${exercise.slug}`}>
